...
func (ps *UI) displayPackages(text string) {
	...
	go func() {
		...
		// search repositories
		packages, localPackages, err = ps.pkgSearch(text)
		if err != nil {
...
func (ps *UI) displayPackageInfo(row, column int) {
	...
	var info *Package = nil

	showFunc := func() {
		for _, shown := range ps.shownPackages {
			if shown.AppID == pkg {
				info = &shown
				break
			}
		...
		if info == nil {
			...
			ps.tableDetails.SetCellSimple(0, 0, "[red]Package not found")
			return
		}
		ps.selectedPackage = info
		ps.drawPackageInfo(*info, ps.width)
	}

	// if infoCached, found := ps.cacheInfo.Get(pkg + "-" + source); found {
	// 	info = []Package{infoCached.(InfoRecord)}
	// 	showFunc()
	// 	return
	// }
	...
	go func() {
		...
		if !ps.conf.DisableCache && info != nil {
			ps.cacheInfo.Set(pkg+"-"+info.Remote, info, time.Duration(ps.conf.CacheExpiry)*time.Minute)
		}
...
func (ps *UI) displayHelp() {
	...
	ps.tableDetails.SetCellSimple(0, 0, "ENTER: Search; Install or remove a selected package").
		...
		SetCell(16, 0, &tview.TableCell{
			...
			Clicked: func() bool {
				exec.Command("xdg-open", "https://github.com/XnLogicaL/flatseek/wiki/Usage").Start()
				return true
			},
		})
}
...
func (ps *UI) displayAbout() {
	...
	ps.tableDetails.Clear().
		...
		SetCellSimple(2, 1, "https://github.com/XnLogicaL/flatseek")
